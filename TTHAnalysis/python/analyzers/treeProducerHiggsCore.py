...
higgsCore_globalVariables = [
            ...
            NTupleVariable('passBadMuonFilter', lambda ev : ev.passBadMuonFilter if hasattr(ev, 'passBadMuonFilter') else -1, int),
            NTupleVariable('passBadChargedHadronFilter', lambda ev : ev.passBadChargedHadronFilter if hasattr(ev, 'passBadChargedHadronFilter') else -1, int),
            NTupleVariable('failBadGlobalMuonTagger', lambda ev : ev.failBadGlobalMuonTaggerMAOD if hasattr(ev, 'failBadGlobalMuonTaggerMAOD') else -1, int),
            NTupleVariable('failCloneGlobalMuonTagger', lambda ev : ev.failCloneGlobalMuonTaggerMAOD if hasattr(ev, 'failCloneGlobalMuonTaggerMAOD') else -1, int),
            NTupleVariable('Flag_HBHENoiseFilter', lambda ev: ev.Flag_HBHENoiseFilter if hasattr(ev, 'Flag_HBHENoiseFilter') else -1, int),
            NTupleVariable('Flag_HBHENoiseIsoFilter', lambda ev: ev.Flag_HBHENoiseIsoFilter if hasattr(ev, 'Flag_HBHENoiseIsoFilter') else -1, int),
            NTupleVariable('Flag_EcalDeadCellTriggerPrimitiveFilter', lambda ev: ev.Flag_EcalDeadCellTriggerPrimitiveFilter if hasattr(ev, 'Flag_EcalDeadCellTriggerPrimitiveFilter') else -1, int),
            NTupleVariable('Flag_goodVertices', lambda ev: ev.Flag_goodVertices if hasattr(ev, 'Flag_goodVertices') else -1, int),
            NTupleVariable('Flag_eeBadScFilter', lambda ev: ev.Flag_eeBadScFilter if hasattr(ev, 'Flag_eeBadScFilter') else -1, int),
            NTupleVariable('Flag_globalTightHalo2016Filter', lambda ev: ev.Flag_globalTightHalo2016Filter if hasattr(ev, 'Flag_globalTightHalo2016Filter') else -1, int),
            NTupleVariable('Flag_duplicateMuons', lambda ev: ev.Flag_duplicateMuons if hasattr(ev, 'Flag_duplicateMuons') else -1, int),
            NTupleVariable('Flag_badMuons', lambda ev: ev.Flag_badMuons if hasattr(ev, 'Flag_badMuons') else -1, int),
            NTupleVariable('Flag_noBadMuons', lambda ev: ev.Flag_noBadMuons if hasattr(ev, 'Flag_noBadMuons') else -1, int),
